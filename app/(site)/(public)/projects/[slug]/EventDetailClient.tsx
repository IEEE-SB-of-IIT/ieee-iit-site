...
const SOCIETY_ORGANIZERS: Record<string, string> = {
  CS: "IEEE CS of IIT",
  RAS: "IEEE RAS of IIT",
  WIE: "IEEE WIE of IIT",
  CIS: "IEEE CIS of IIT",
  EMBS: "IEEE EMBS of IIT",
  SB: "IEEE Student Branch of IIT",
};

const AnimatedGallery = ({ images, eventName }: { images: string[]; eventName: string }) => {
  const [displayImages, setDisplayImages] = React.useState<string[]>(images.slice(0, 6));
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-3 gap-4" style={{ perspective: "1000px" }}>
      <div className={`col-span-2 row-span-2 group relative rounded-2xl overflow-hidden cursor-pointer transition-all duration-500 ease-in-out transform-gpu ${getTransitionClasses(0)}`}>
        <img src={slots[0]} alt={`${eventName} featured`} loading="lazy" decoding="async" className="w-full h-full object-cover min-h-[300px] md:min-h-[500px] transition-transform duration-700 group-hover:scale-105" />
      ...
      {slots.slice(1, 3).map((img, idx) => (
        <div key={`top-${idx}`} className={`group relative rounded-2xl overflow-hidden cursor-pointer transition-all duration-500 ease-in-out transform-gpu ${getTransitionClasses(idx + 1)}`}>
          <img src={img} alt={`${eventName} gallery ${idx + 2}`} loading="lazy" decoding="async" className="w-full h-full object-cover min-h-[200px] md:min-h-[240px] transition-transform duration-700 group-hover:scale-105" />
          <div className="absolute inset-0 bg-gradient-to-t from-[#0a1628]/50 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
        </div>
      ))}
      {slots.slice(3, 6).map((img, idx) => (
        <div key={`bottom-${idx}`} className={`group relative rounded-2xl overflow-hidden cursor-pointer transition-all duration-500 ease-in-out transform-gpu ${getTransitionClasses(idx + 3)}`}>
          <img src={img} alt={`${eventName} gallery ${idx + 4}`} loading="lazy" decoding="async" className="w-full h-full object-cover min-h-[200px] md:min-h-[240px] transition-transform duration-700 group-hover:scale-105" />
          <div className="absolute inset-0 bg-gradient-to-t from-[#0a1628]/50 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
        </div>
      ))}
    </div>
...
export default function EventDetailClient({ event }: { event: Project }) {
  const organizer = SOCIETY_ORGANIZERS[event.society] || SOCIETY_ORGANIZERS.SB;
    ...
    <div className="relative min-h-screen overflow-hidden">
      ...
      <div className="relative z-10">
        <div className="relative w-full h-[55vh] md:h-[65vh] overflow-hidden">
          <Image src={headerImage} alt={event.name} fill priority className="object-cover" />
          ...
          <Link href="/projects" className="absolute top-32 md:top-40 left-6 md:left-12 flex items-center gap-2 bg-white/10 backdrop-blur-xl text-white/90 px-5 py-2.5 rounded-full text-sm font-medium border border-white/10 hover:bg-white/20 hover:border-white/30 transition-all duration-300 group">
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-12 lg:px-16 py-16">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 mb-20">
            <div className="lg:col-span-2">
              <h2 className="text-2xl md:text-3xl font-bold text-[#0d2440] mb-6">About This Event</h2>
            ...
            <div className="space-y-4">
              <div className="bg-white/60 backdrop-blur-sm border border-white/80 rounded-2xl p-6 shadow-sm">
                <h3 className="text-lg font-bold text-[#0d2440] mb-5">Event Details</h3>
                <div className="space-y-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="text-sm text-[#2e5e99]/60 font-medium">Date</p>
                      <p className="text-[#0d2440] font-semibold">{event.date}</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    <div className="w-10 h-10 rounded-xl bg-[#00629B]/10 flex items-center justify-center flex-shrink-0">
                    ...
                    <div>
                      <p className="text-sm text-[#2e5e99]/60 font-medium">Venue</p>
                      <p className="text-[#0d2440] font-semibold">Informatics Institute of Technology</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    <div className="w-10 h-10 rounded-xl bg-[#00629B]/10 flex items-center justify-center flex-shrink-0">
                    ...
                    <div>
                      <p className="text-sm text-[#2e5e99]/60 font-medium">Organized By</p>
                      <p className="text-[#0d2440] font-semibold">{organizer}</p>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="mb-20">
            <h2 className="text-2xl md:text-3xl font-bold text-[#0d2440] mb-3">Event Gallery</h2>
            <p className="text-[#2e5e99] mb-10">Highlights and moments captured during the event.</p>
